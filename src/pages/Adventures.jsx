...
import adventures_hero from '../assets/images/adventures_hero.png';
...
export default function Adventures() {
    ...
    // Determine heading based on current path
    // The user requested: /destinations/india-corner => "ITINERARIES"
    // Otherwise => "ADVENTURES"
    // Note: location.pathname might be case sensitive, checking lowercase for robustness if needed, 
    // but usually paths are exact. User wrote "/Destination/india-corner" in their bad code, 
    // but "/destinations/india-corner" in text request. I will check both or normalize.
    // Assuming standard lowercase path from IndiaCorner usage earlier: "/destinations/india-corner"

    const isIndiaCorner = location.pathname.toLowerCase().includes('india-corner');
    console.log("isIndiaCorner", isIndiaCorner);
    const heading = isIndiaCorner ? "ITINERARIES" : "ADVENTURES";

    useEffect(() => {
        const fetchBlogs = async () => {
            try {
                const res = await axios.get('http://localhost:4000/api/blogs?tag=adventures');
                setBlogs(res.data);
            } catch (err) {
    ...
    useEffect(() => {
        const fetchExploreBlogs = async () => {
            try {
                const res = await axios.get('http://localhost:4000/api/blogs?tag=exploreadventures');
        ...
        <>
            <div className="flex flex-col h-[50vh] md:h-[680px]">
                <div
                    ...
                    style={{ backgroundImage: `url(${adventures_hero})` }}
                >
